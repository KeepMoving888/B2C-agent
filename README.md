...
基于多智能体（LangGraph）+ RAG + Qwen2.5-7B QLoRA 微调 + AWQ 量化 + vLLM 高并发推理的跨境 B2C 多语言智能客服开源参考实现。

覆盖欧美、东南亚 12 国 8 种语言，支持独立站 + 亚马逊等多平台会话统一接待，提供咨询、订单、售后、合规、人工转接 5 大核心智能体的全流程自动化编排。
...
三栏布局：左侧会话列表 / 中间聊天流 / 右侧客户详情。顶部支持 6 大平台切换（Amazon / 速卖通 / eBay / 独立站 / 乐天 / 邮件），红点标注未读消息数。

![系统首页](docs/images/homepage.png)

### 会话列表与阶段标签

每个会话项标注客户业务阶段（未下单 / 已下单 / 售后 / 复购 / 投诉升级），对应 5 大核心智能体（咨询 Agent / 订单 Agent / 售后 Agent / 复购 Agent / 人工转接 Agent），直观展示多 Agent 路由状态。

![会话列表](docs/images/conversation.png)

### AI 分析建议

客服点击「AI 分析建议」按钮，系统基于 RAG 检索 + 反幻觉校验生成推荐回复，支持一键采纳或编辑后发送。

![AI分析建议](docs/images/ai_suggestion.png)

### 多平台切换

点击顶部平台标签切换不同平台的会话列表，各平台独立统计未读消息数。

![平台切换](docs/images/platform_switch.png)

### 客户详情面板

右侧展示客户信息（真实外贸客户名 / 国家 / 语言 / 历史订单 / 阶段标签），辅助客服快速了解客户背景。

![客户详情](docs/images/customer_detail.png)
...
本项目为开源参考实现，提供完整技术链路与可运行代码，便于学习与二次开发。生产级商用方案（含真实平台 API 对接、私有知识库、模型权重、监控告警、多租户等）不在开源范围内。
